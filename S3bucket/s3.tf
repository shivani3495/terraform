...
resource "aws_s3_bucket" "statichp21" {
    bucket = "statichp21"
 ...
 provisioner "local-exec" {
    command = "git clone https://github.com/GauravRaturi/Pratice web-server-image"
 }
}
#Allow public access to the bucket
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.statichp21]
 bucket = "statichp21"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.statichp21.bucket_regional_domain_name
